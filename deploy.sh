...
if [ "$DEPLOY_PYTHON_BACKEND" = "yes" ]; then
  ...
  echo "  ./start.sh    # test once manually — Ctrl+C if you see Uvicorn running"
  echo "  pm2 delete portfolio-rag-api 2>/dev/null || true"
  echo "  pm2 start ./start.sh --name portfolio-rag-api --interpreter bash"
  echo "  pm2 save"
  echo "  pm2 status    # should show fork + online (not cluster + errored)"
  echo ""
  echo "  Test on server:"
  echo "  curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:8000/docs"
  echo "  (should print 200 — if connection refused, API is not running)"
  echo "------------------------------------------------------------------"
  echo ""
fi
